...
const internCreate = async function (req, res) {
    ...
    try {
       ...
       // validation user entry or  req.body
        ...
        let check2 = (/^[6-9]{1}\d{9}$/
        .test(requestBody.mobile))
        ...
        const collegeId = await collegeModel.findOne({ _id: requestBody.collegeId })
        if (!collegeId) {
        ...
        // create data in intern model first method
